...
abstract contract Base is AzosTest {
  ...
  IBaseOracle mockToken0PriceSource = IBaseOracle(mockContract('Token0PriceSource'));
  IBaseOracle mockToken1PriceSource = IBaseOracle(mockContract('Token1PriceSource'));
...
contract Unit_BeefyVeloVaultRelayerFactory_DeployBeefyVeloVaultRelayer is Base {
  ...
  function test_Deploy_BeefyVeloVaultRelayerChild(
    ...
    // params
    assertEq(address(beefyVeloVaultRelayerChild.token0priceSource()), address(mockToken0PriceSource));
    assertEq(address(beefyVeloVaultRelayerChild.token1priceSource()), address(mockToken1PriceSource));
  ...
  function test_Emit_NewBeefyVeloVaultRelayer(
    ...
    emit NewBeefyVeloVaultRelayer(
      ...
      address(mockToken0PriceSource),
      address(mockToken1PriceSource),
      address(mockBeefyVault),
      address(mockVeloPool)
    );
